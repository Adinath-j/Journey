import { BarChart3, Check, Circle, Lightbulb, Quote, Sparkles, Target } from "lucide-react";
import { MetricCard } from "@/components/common/MetricCard";
import { SectionTitle } from "@/components/common/SectionTitle";
import { GlassCard } from "@/components/ui/GlassCard";
import { ProgressBar } from "@/components/ui/ProgressBar";
import { PageContainer } from "@/components/layout/PageContainer";
import { dailyTip, dashboardOverview, focusItems, metrics, missionItems, quickStats, quote } from "@/data/mock/dashboard";
  ...
  return (
    <PageContainer>
      <section className="mb-5 flex items-start justify-between px-2">
        <div>
          <h1 className="text-[27px] font-semibold tracking-tight text-slate-50">{dashboardOverview.greeting}, {dashboardOverview.userName}! <span aria-hidden="true">👋</span></h1>
          <p className="mt-2 flex items-center gap-2 text-[13px] text-slate-300"><Sparkles className="size-4 text-violet-400" />{dashboardOverview.subtitle} <span aria-hidden="true">✨</span></p>
        </div>
        <time className="pt-1 text-right text-xs leading-7 text-slate-300"><span className="block">Sunday, 25 May 2025</span><strong className="text-[23px] font-normal tracking-wide">08:42 PM</strong></time>
      </section>
      ...
      <section className="mt-4 grid grid-cols-[1.08fr_1.08fr_.85fr] gap-4">
        <GlassCard className="flex min-h-[250px] flex-col overflow-hidden p-5">
          ...
          <div className="quote-card -mx-5 -mb-5 mt-auto flex min-h-[85px] items-center gap-3 border-t border-violet-400/15 px-6 py-4">
            ...
            <p className="text-[13px] leading-5 text-violet-300">{quote.text}<span className="mt-1 block text-[10px] text-slate-300">— {quote.author}</span></p>
          </div>
        </GlassCard>
        ...
        <GlassCard className="flex min-h-[250px] flex-col p-5">
          ...
          <div className="mt-auto flex items-center justify-between border-t border-white/[0.06] pt-4 text-[12px]"><span>Estimated Time</span><span className="text-violet-300">~ {dashboardOverview.estimatedTime}</span></div>
        </GlassCard>
        ...
        <div className="flex flex-col gap-4">
          <GlassCard className="p-5">
            ...
            <div className="mt-4 space-y-3.5">
              {quickStats.map((stat) => { const Icon = stat.icon; return <div key={stat.label} className="flex items-center gap-3 text-[11px]"><Icon className={`size-4 ${stat.accent}`} /><span className="flex-1 text-slate-300">{stat.label}</span><span className="text-slate-100">{stat.value}</span></div>; })}
            </div>
          </GlassCard>
      ...
      <section className="mt-4 grid grid-cols-[1fr_.26fr] gap-4 2xl:grid-cols-1">
        ...
        <GlassCard className="flex flex-col justify-center p-5 2xl:hidden">
          ...
          <p className="mt-4 text-sm leading-6 text-slate-100">{dailyTip}</p>
